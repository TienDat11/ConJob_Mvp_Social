        ...
        name:
          selectedUsers.length > 1
            ? loggedInUser.displayName +
              ", " +
              selectedUsers.map((u) => u.name).join(", ") + "phòng chat tuyệt vời nhất thế giới"
            : undefined,
      });
...
function UserResult({ user, selected, onClick }: Readonly<UserResultProps>) {
  return (
    <button
...
function SelectedUserTag({ user, onRemove }: Readonly<SelectedUserTagProps>) {
  return (
    <button
      onClick={onRemove}
